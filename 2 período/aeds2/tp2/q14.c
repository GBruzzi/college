...
int getCharIndex(char *str, int pos) {
    if (pos < strlen(str)) {
        return str[pos];
    }
    return 0; // Retorna 0 se a posição exceder o tamanho da string.
}


void countingSort(Pokemon pokedex[], int n, int index, int byAbilities) {
    Pokemon output[n];
    int count[256] = {0};  // Para caracteres ASCII

    // Contagem da frequência de cada caractere (abilities ou name)
    for (int i = 0; i < n; i++) {
        char *key = byAbilities ? pokedex[i].abilities : pokedex[i].name;
        int charIdx = getCharIndex(key, index);
        count[charIdx]++;
    }

    // Ajuste dos índices no array de contagem
    for (int i = 1; i < 256; i++) {
        count[i] += count[i - 1];
    }

    // Coloca os Pokémons na ordem correta
    for (int i = n - 1; i >= 0; i--) {
        char *key = byAbilities ? pokedex[i].abilities : pokedex[i].name;
        int charIdx = getCharIndex(key, index);
        output[count[charIdx] - 1] = pokedex[i];
        count[charIdx]--;
    }

    // Copia a ordem ordenada de volta ao array original
    for (int i = 0; i < n; i++) {
        pokedex[i] = output[i];
    }
}
...
void radixSort(Pokemon pokedex[], int n) {
    int maxLenAbilities = 0, maxLenName = 0;

    // Calcula o tamanho máximo de abilities e name
    for (int i = 0; i < n; i++) {
        int lenAbilities = strlen(pokedex[i].abilities);
        int lenName = strlen(pokedex[i].name);
        if (lenAbilities > maxLenAbilities) maxLenAbilities = lenAbilities;
        if (lenName > maxLenName) maxLenName = lenName;
    }

    // RadixSort baseado no nome primeiro (critério de desempate)
    for (int i = maxLenName - 1; i >= 0; i--) {
        countingSort(pokedex, n, i, 0);  // 0 significa que estamos ordenando por name
    }

    // RadixSort baseado nas abilities (chave primária)
    for (int i = maxLenAbilities - 1; i >= 0; i--) {
        countingSort(pokedex, n, i, 1);  // 1 significa que estamos ordenando por abilities
    }
}
...
int main() {
    ...
    // Chame a função radixSort
    radixSort(findPokemon, j);
    ...
    FILE *arquivo = fopen("813584_radixsort.txt", "w");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo!\n");
        return 1;
    }

    fprintf(arquivo, "813584\t%.2f\n", executionTime);
    fclose(arquivo);
