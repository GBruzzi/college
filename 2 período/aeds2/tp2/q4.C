...
void ordenarPorNome(Pokemon *pokedex, int n) {
    for (int i = 1; i < n; i++) {
        Pokemon key = pokedex[i];
        int j = i - 1;

        // Move os elementos de pokedex[0..i-1] que são maiores que key.nome
        while (j >= 0 && strcmp(pokedex[j].name, key.name) > 0) {
...
int main() {
    ...
    // Chamada da função de ordenação por nome
    ordenarPorNome(findPokemon, j);
