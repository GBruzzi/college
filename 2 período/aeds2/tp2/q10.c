...
// Função ShellSort
void quickSort(Pokemon pokedex[], int low, int high, int comp[], int mov[]) {
    if (low < high) {
        int pivotIndex = partition(pokedex, low, high, comp, mov);
        quickSort(pokedex, low, pivotIndex - 1, comp, mov);
        quickSort(pokedex, pivotIndex + 1, high, comp, mov);
    }
}

int partition(Pokemon pokedex[], int low, int high, int comp[], int mov[]) {
    Pokemon pivot = pokedex[high];
    int i = low - 1;

    for (int j = low; j < high; j++) {
        // Compara com base na geração e, em caso de empate, no nome
        if (pokedex[j].generation < pivot.generation ||
            (pokedex[j].generation == pivot.generation && strcmp(pokedex[j].name, pivot.name) < 0)) {
            i++;
            // Troca os Pokémon
            Pokemon temp = pokedex[i];
            pokedex[i] = pokedex[j];
            pokedex[j] = temp;
            mov[0] += 3; // 3 movimentos: 2 para trocar e 1 para o índice
        }
        comp[0]++;
    }
    
    // Troca o pivô com o último elemento menor
    Pokemon temp = pokedex[i + 1];
    pokedex[i + 1] = pokedex[high];
    pokedex[high] = temp;
    mov[0] += 3; // 3 movimentos para a troca do pivô
...
int main () {
    ...
    lerPokemon(file, pokedex, &n); // Suponha que esta função já está implementada
    ...
    // Chame a função quickSort em vez de shellSort
    quickSort(findPokemon, 0, j - 1, comp, mov);

    for (int i = 0 ; i < j ; i++) {
        imprimirPokemon(&findPokemon[i]); // Suponha que esta função já está implementada
    }
    ...
    FILE *arquivo = fopen("813584_quicksort.txt", "w"); 
    if (arquivo == NULL) {
