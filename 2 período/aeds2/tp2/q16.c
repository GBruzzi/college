...
// Função para ordenação Insertion Sort com base em captureDate
void insertionSortByDate(Pokemon pokedex[], int n) {
    for (int i = 1; i < n; i++) {
        Pokemon key = pokedex[i];
        int j = i - 1;

        // Compara a data de captura
        while (j >= 0 && compareDates(pokedex[j].captureDate, key.captureDate) > 0) {
...
int main() {
    ...
    Pokemon pokedex[801]; // Supondo que há até 801 Pokémons
    int n = 0;

    lerPokemon(file, pokedex, &n); // Função que lê os dados do CSV e preenche o array pokedex
    fclose(file);

    // Ordena toda a lista de Pokémons por data de captura
    insertionSortByDate(pokedex, n);

    // Imprime apenas os primeiros 10 Pokémons após a ordenação
    for (int i = 0; i < n && i < 10; i++) {
        imprimirPokemon(&pokedex[i]);
    }

    // Libera a memória usada (para strings dinâmicas)
    for (int i = 0; i < n; i++) {
        free(pokedex[i].name);
    }
