...
// Função para comparar os Pokémons pelo atributo 'height' (desempate por 'name')
...
// Função para aplicar o HeapSort
void heapSort(Pokemon arr[], int n) {
    // Constrói o heap (rearranjo o array)
...
int main() {
    ...
    // Filtra os Pokémons baseados nos IDs fornecidos
    ...
    // Ordena os Pokémons encontrados utilizando o HeapSort pelo atributo height (desempate por name)
    heapSort(findPokemon, j);

    // Imprime os Pokémons encontrados após a ordenação
    for (int i = 0; i < 10; i++) {
