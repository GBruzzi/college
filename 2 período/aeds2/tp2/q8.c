...
// Função ShellSort
void shellSort(Pokemon pokedex[], int n, int comp[], int mov[]) {
    for (int gap = n / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < n; i++) {
            Pokemon temp = pokedex[i];
            int j;
            for (j = i; j >= gap && 
                (temp.weight < pokedex[j - gap].weight || 
                (temp.weight == pokedex[j - gap].weight && strcmp(temp.name, pokedex[j - gap].name) < 0)); 
                j -= gap) {
...
int main () {
    ...
    // Chame a função shellSort em vez de selectionSort
    ...
    FILE *arquivo = fopen("813584_shellsort.txt", "w");
    if (arquivo == NULL) {
