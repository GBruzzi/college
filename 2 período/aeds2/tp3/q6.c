...
typedef struct Celula {
    Pokemon *pokemon;        // Ponteiro para o Pokémon
...
typedef struct Pilha {
    ...
    int tam;                 // Tamanho da pilha (número de elementos)
} Pilha;

// Função para inicializar a pilha
Pilha inicializarPilha() {
    Pilha pilha;
    pilha.topo = NULL;  // Inicialmente a pilha está vazia
    pilha.tam = 0;      // Tamanho 0
    return pilha;
}

// Função para empilhar um Pokémon (inserir no topo da pilha)
void push(Pilha *pilha, Pokemon *pokemon) {
    Celula *novaCelula = (Celula*)malloc(sizeof(Celula));  // Aloca uma nova célula
    novaCelula->pokemon = pokemon;  // Atribui o Pokémon à nova célula
    novaCelula->prox = pilha->topo;  // A nova célula aponta para o topo atual da pilha
    pilha->topo = novaCelula;  // O topo da pilha agora é a nova célula
    pilha->tam++;  // Incrementa o tamanho da pilha
}

// Função para desempilhar um Pokémon (remover do topo da pilha)
Pokemon* pop(Pilha *pilha) {
    if (pilha->topo == NULL) {
        printf("A pilha está vazia\n");
        return NULL;
    }
    Celula *celulaRemovida = pilha->topo;  // A célula a ser removida
    pilha->topo = celulaRemovida->prox;  // O topo da pilha passa a ser a célula seguinte
    Pokemon *pokemon = celulaRemovida->pokemon;  // O Pokémon removido
    free(celulaRemovida);  // Libera a célula removida
...
void imprimirPilha(Pilha *pilha) {
    ...
    Pokemon *pokemons[pilha->tam];  
    int i = 0;
    
    // Armazenando os Pokémons em um vetor
    while (atual != NULL) {
        pokemons[i] = atual->pokemon;
        atual = atual->prox;
        i++;
    }

    // Imprimindo os Pokémons na ordem de inserção (do fundo para o topo)
    for (int j = pilha->tam - 1; j >= 0; j--) {  // Percorre o vetor de trás para frente
        printf("[%d] ", pilha->tam - j - 1);
        imprimirPokemon(pokemons[j]);  // Função para imprimir o Pokémon
    }
}
...
int main() {
    ...
    Pilha pilhaPokemons = inicializarPilha();  // Inicializa a pilha (agora flexível)

    // Lê IDs de Pokémon até a entrada "FIM"
    char inputId[10];
    scanf("%s", inputId);
    ...
    for (int i = 0; i < qtdComandos; i++) {
        ...
        if (strcmp(comando, "I") == 0) {
            // Inserir Pokémon no topo da pilha
            scanf("%s", inputId);
            int id = atoi(inputId);
            Pokemon* p = findPokemonById(pokedex, n, id);
            if (p != NULL) {
            ...
            // Remover Pokémon do topo da pilha
            Pokemon* p = pop(&pilhaPokemons);
            if (p != NULL) {
                printf("(R) %s\n", p->name);
            }
        }
    ...
    // Imprime todos os Pokémons na pilha
    imprimirPilha(&pilhaPokemons);

    // Liberação de memória dos nomes e descrições dos Pokémons
    for (int i = 0; i < n; i++) {
        free(pokedex[i].name);
